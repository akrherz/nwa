...
$rs = pg_query($conn, "SELECT team, count(*) from nwa_warnings ".
		"where issue >= '2022-04-21 19:30+00' and obs is not null ".
		" and issue < '2022-04-21 21:00+00' ".
...
for ($i=0;$row = pg_fetch_array($rs);$i++)
{
	$total += $row["count"];
   if ($i % 3 == 0){ echo "<tr>"; }
   $uri = sprintf("http://workshop.agron.iastate.edu/auto/teamwarns_%s.png", urlencode($row["team"]));
   $html = sprintf(
